...
import { LayoutDashboard, Users, ShoppingBag, ShoppingCart, Menu, X, WashingMachine, Bell, PieChart, LogOut } from 'lucide-react';
      ...
      <div className="flex-1 flex flex-col relative min-w-0 z-10">
        <header className="relative z-50 h-24 px-6 lg:px-10 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center space-x-4 ml-auto">
            ...
            >
              <div className="absolute top-2.5 right-2.5 w-2 h-2 bg-red-500 rounded-full animate-ping" />
              ...
              <div className="flex items-center gap-3.5 bg-white p-2 pr-4 rounded-2xl border border-slate-200 shadow-sm cursor-pointer hover:shadow-md transition-all">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-lg shadow-inner overflow-hidden shrink-0">
                  {tenant.profilePictureUrl ? (
                    <img src={tenant.profilePictureUrl} alt="avatar" className="w-full h-full object-cover" />
                  ) : (
                    <span>{tenant.ownerName?.charAt(0) || 'A'}</span>
                  )}
                </div>
                <div className="hidden sm:block text-left">
                  <p className="text-sm font-bold text-slate-900 leading-tight truncate max-w-[120px]">{tenant.ownerName || 'Owner'}</p>
                  <p className="text-xs text-slate-500 leading-tight truncate max-w-[120px]">{tenant.storeName || 'Kasir Utama'}</p>
                </div>
              </div>
            </Link>
          </div>
