...
import {
  ...
  WashingMachine, Bell, PieChart, LogOut, ChevronLeft, ChevronRight,
} from 'lucide-react';
    ...
    <div className="flex h-screen bg-gradient-to-br from-blue-50/50 via-slate-50 to-indigo-50/50 font-sans overflow-hidden text-slate-800 transition-colors duration-300">
      ...
      <motion.aside
        ...
        className={cn(
          ...
          "bg-white/90 backdrop-blur-2xl border border-white shadow-xl shadow-blue-500/5",
      ...
      >
        <div className={cn("p-6 flex items-center", isCollapsed ? "justify-center" : "justify-between")}>
          <div className={cn("flex items-center gap-3", isCollapsed && "flex-col")}>
            ...
            {!isCollapsed && (
              <div>
                <span className="text-lg font-display font-bold text-slate-900 tracking-tight block truncate max-w-[130px]">
                  {tenant.storeName || 'Laundry'}
                </span>
                <span className="text-[10px] font-semibold text-indigo-600 uppercase tracking-wider block">MITRA UMKM</span>
              </div>
            )}
          </div>
          {!isCollapsed && (
            <button className="lg:hidden text-slate-400 hover:text-slate-800" onClick={() => setIsSidebarOpen(false)}>
              <X size={20} />
            </button>
          )}
        </div>
            ...
            return (
              ...
              >
                ...
                <div className={cn(
                  ...
                  <Icon size={20} className={cn("shrink-0", isActive ? "text-blue-600" : "text-slate-400")} />
        ...
        <div className={cn("p-4 mt-auto space-y-2", isCollapsed && "flex flex-col items-center")}>
          ...
          <button
            ...
            className="hidden lg:flex w-full items-center justify-center p-1.5 text-slate-400 hover:text-slate-600 transition-colors cursor-pointer rounded-xl hover:bg-slate-100"
      ...
      <div className="flex-1 flex flex-col relative min-w-0 z-10">
        <header className="relative z-50 h-20 px-4 lg:px-8 flex items-center justify-between shrink-0">
          <button
            className="p-2.5 text-slate-600 bg-white shadow-sm rounded-xl lg:hidden hover:bg-slate-50 border border-slate-200 transition-all cursor-pointer"
          ...
          <div className="flex items-center gap-3 ml-auto">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="relative p-2.5 text-slate-600 bg-white shadow-sm rounded-xl border border-slate-200 transition-all cursor-pointer"
            >
            ...
            <Link to="/profile">
              <div className="flex items-center gap-2.5 bg-white p-1.5 pr-3 rounded-xl border border-slate-200 shadow-sm cursor-pointer hover:shadow-md transition-all">
                ...
                <div className="hidden sm:block text-left">
                  <p className="text-xs font-bold text-slate-900 leading-tight truncate max-w-[100px]">{tenant.ownerName || 'Owner'}</p>
                  <p className="text-[10px] text-slate-500 leading-tight truncate max-w-[100px]">{tenant.storeName || 'Kasir'}</p>
                </div>
              </div>
            </Link>
          </div>
