      ...
      >
        ...
        <div className="max-w-3xl mx-auto relative z-10">
          ...
          <p className="text-blue-100 text-lg font-medium">
            Nomor NOTA:{' '}
            <span className="font-mono bg-white/20 px-3.5 py-1 rounded-xl font-bold tracking-wider ml-1">{order.id}</span>
          </p>
        </div>
      </motion.div>
                    ...
                    <div
                      className={cn(
                        ...
                        isCompleted
                          ...
                          : isCurrent
                          ? 'border-blue-600 bg-blue-600 text-white scale-110 shadow-lg shadow-blue-600/30 ring-4 ring-blue-100'
                      ...
                      <h3
                        className={cn(
                          ...
                          isCompleted
                            ? 'text-slate-800'
                            : isCurrent
                            ? 'text-blue-600 text-lg sm:text-xl font-extrabold'
                            : 'text-slate-400'
                        )}
                      >
                        {statusIndo[status]}
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-3 pt-1">
              {order.orderItems?.map((item) => (
                <div
                  key={item.id}
                  className="flex justify-between items-center bg-white border border-slate-100 shadow-sm p-4 rounded-2xl hover:border-blue-200 transition-all"
                >
                  <div className="flex flex-col">
                    <span className="font-bold text-slate-800 text-base">{item.service?.name || 'Paket Cucian'}</span>
                    <span className="text-slate-500 font-medium text-xs mt-0.5">
                      {item.quantity} {item.service?.unit || 'x'}
                    </span>
                  </div>
