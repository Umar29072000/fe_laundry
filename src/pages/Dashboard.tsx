...
export default function Dashboard() {
  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      if (data.status === 'success' && data.data) setStats(data.data);
      else setError(data.message || 'Gagal memuat data dashboard');
  ...
  const revenueCards = [
    { title: 'Pendapatan Hari Ini', value: formatRupiah(stats.todayRevenue), icon: DollarSign, color: 'from-blue-500 to-indigo-600', shadow: 'shadow-blue-500/20' },
    { title: 'Pendapatan Minggu Ini', value: formatRupiah(stats.weeklyRevenue), icon: Calendar, color: 'from-purple-500 to-pink-600', shadow: 'shadow-purple-500/20' },
    { title: 'Pendapatan Bulan Ini', value: formatRupiah(stats.monthlyRevenue), icon: TrendingUp, color: 'from-emerald-500 to-teal-600', shadow: 'shadow-emerald-500/20' },
    { title: 'Total Keseluruhan', value: formatRupiah(stats.totalRevenue), icon: TrendingUp, color: 'from-amber-500 to-orange-600', shadow: 'shadow-amber-500/20' },
  ];

  const summaryCards = [
    { title: 'Total Pelanggan', value: stats.totalCustomers, icon: Users, color: 'from-sky-500 to-cyan-600', shadow: 'shadow-sky-500/20', link: '/customers' },
    { title: 'Total Pesanan', value: stats.totalOrders, icon: ShoppingCart, color: 'from-violet-500 to-purple-600', shadow: 'shadow-violet-500/20', link: '/orders' },
    { title: 'Pesanan Aktif', value: stats.pendingOrdersCount, icon: Clock, color: 'from-amber-400 to-orange-500', shadow: 'shadow-orange-500/20', link: '/orders' },
    { title: 'Buka Laporan', value: 'Detail →', icon: Sparkles, color: 'from-emerald-400 to-teal-500', shadow: 'shadow-emerald-500/20', link: '/reports' },
  ];

  const container = { hidden: { opacity: 0 }, show: { opacity: 1, transition: { staggerChildren: 0.06 } } };
  const itemAnim = { hidden: { opacity: 0, y: 15 }, show: { opacity: 1, y: 0, transition: { type: 'spring' as const, stiffness: 300, damping: 24 } } };
  ...
  return (
    <div className="space-y-6 pt-1 pb-10">
      ...
      {/* Revenue Cards */}
      <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
